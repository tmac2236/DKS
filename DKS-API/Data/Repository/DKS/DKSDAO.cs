...
namespace DFPS.API.Data.Repository
{
    public class DKSDAO : IDKSDAO
    {
        ...
        public async Task<List<UserRoleDto>> GetRolesByAccount(string account)
        {
            string strWhere = "WHERE A.LOGIN='" + account + "'" ;
            string strSQL = string.Format(@"
                                            SELECT  
                                                   A.WORKPNO
                                                  ,CONVERT(varchar,A.USERID) AS USERID
...
ON t1.PKBASEHID=t3.FKBASEHID and t3.LANGID='437' ");
        ...
        public async Task<List<DevBomFileDetailDto>> GetDevBomFileDto(SDevBomFile sDevBomFile)
        {
            List<SqlParameter> pc = new List<SqlParameter>{
            ...
            var data = await _context.GetDevBomFileDetailDto
                   .FromSqlRaw("EXECUTE dbo.GetDevBomFileDetail @FACTORYID,@SEASON,@MODELNO,@MODELNAME,@ARTICLE,@DEVTEAMID", pc.ToArray())
                   .ToListAsync();
        ...
        public async Task<List<SendDevBomDetailMailListDto>> GetSendDevBomDetailMailListDto(string stage,string factory,string devTeamId)
        {
            List<SqlParameter> pc = new List<SqlParameter>{
                new SqlParameter("@STAGE",stage),
                new SqlParameter("@DEVTEAMID",devTeamId),
                new SqlParameter("@FACTORY",factory)
            };
            var data = await _context.SendDevBomDetailMailListDto
                   .FromSqlRaw("EXECUTE dbo.SendDevBomDetailMailList @STAGE,@DEVTEAMID,@FACTORY", pc.ToArray())
                   .ToListAsync();
